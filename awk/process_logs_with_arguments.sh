...
# the following actions based on the provided parameter:
#
# Usage:
#   ./process_logs.sh <mode> [filter_string]
#
#   Modes:
#     all      - Extracts logs, runs AWK, and creates Excel file.
#     extract  - Only extracts logs into folders.
#     analyze  - Runs AWK and Python scripts on existing folders.
#
#   Optional [filter_string]:
#     A comma-separated list of strings. When in 'all' or 'analyze' mode, only
#     folders containing log files that match one of these strings will be processed.
#     Example: ./process_logs.sh analyze 2025081208,2025081209
#
# Prerequisites:
...
def main():
    """
    Main function to combine CSVs and format date columns.
    """
    csv_files = [
    ...
    try:
            ...
            for csv_file in csv_files:
                if os.path.exists(csv_file):
                    try:
                        ...
                        # Check for and convert date columns
                        if csv_file in date_columns_map:
                            for col in date_columns_map[csv_file]:
                                if col in df.columns:
                                    df[col] = pd.to_datetime(df[col], errors='coerce')
                                    print(f"  - Converted column '{col}' in '{csv_file}' to datetime.")
        ...
        print(f"\nSuccessfully created final Excel file: {output_excel_file}")
    except Exception as e:
...
usage() {
    echo "Usage: $0 <mode> [filter_string]"
    echo "  Modes:"
    echo "    all      - Extracts logs, runs AWK, and creates the final Excel file."
    echo "    extract  - Only extracts logs into folders."
    echo "    analyze  - Runs AWK and Python scripts on existing folders."
    echo ""
    echo "  Optional [filter_string]:"
    echo "    A comma-separated list of strings to match in filenames."
    echo "    Example: ./process_logs.sh analyze 2025081208,2025081209"
    exit 1
}
...
# Validate the mode
if [[ "$MODE" != "all" && "$MODE" != "extract" && "$MODE" != "analyze" ]]; then
...
check_prerequisites() {
    ...
    for tool in tar xz gawk python3 pip3; do
        if ! command -v "$tool" &> /dev/null; then
            log_with_timestamp "   - ❌ ERROR: Command '$tool' not found. Please install it."
            log_with_timestamp "Aborting due to missing prerequisite."
            exit 1
        fi
...
log_with_timestamp "Starting log processing in '$MODE' mode..."

# --- Main Logic based on Mode ---

if [[ "$MODE" == "all" || "$MODE" == "extract" ]]; then
    # This block handles extraction for 'all' and 'extract' modes
    for file in *.log.txz; do
        [ -e "$file" ] || { log_with_timestamp "No .log.txz files found to process. Exiting."; exit 0; }

        folder_name="${file%.log.txz}"
        log_with_timestamp "▶️  Processing file for extraction: $file"
        ...
        log_with_timestamp "✅ Finished extraction for $file"
        log_with_timestamp "--------------------------------"
    done
fi

if [[ "$MODE" == "all" || "$MODE" == "analyze" ]]; then
    # This block handles analysis for 'all' and 'analyze' modes
    # It iterates over directories, not .txz files
    for dir in */; do
        [ -d "$dir" ] || continue # Skip if not a directory
        
        folder_name=${dir%/} # Remove trailing slash

        # --- Filtering Logic (INSIDE FOLDER) ---
        process_this_folder=true # Default to true if no filter is provided
        if [ -n "$FILTER_STRINGS" ]; then
            process_this_folder=false # If filter exists, default to false until a match is found
            log_with_timestamp "   - Checking contents of '$folder_name' against filter..."
            OLD_IFS=$IFS
            IFS=','
            for filter in $FILTER_STRINGS; do
                if [ -n "$(find "$folder_name" -type f -name "*$filter*")" ]; then
                    log_with_timestamp "   - Match found for filter '$filter'. This folder will be analyzed."
                    process_this_folder=true
                    break
                fi
            done
            IFS=$OLD_IFS

            if [ "$process_this_folder" = false ]; then
                log_with_timestamp "⏩ Skipping analysis for '$folder_name' as no internal files match filter criteria."
                log_with_timestamp "--------------------------------"
                continue
            fi
        fi
        ...
        log_with_timestamp "▶️  Analyzing folder: $folder_name"
        cd "$folder_name" || { log_with_timestamp "   - ❌ ERROR: Could not cd into $folder_name. Skipping."; cd ..; continue; }

        # --- NEW: Build the list of files to be processed by AWK ---
        files_to_process=""
        if [ -n "$FILTER_STRINGS" ]; then
            find_args=()
            OLD_IFS=$IFS
            ...
            IFS=$OLD_IFS
            # Find files in subdirectories (e.g., cpsns/, testeng/) that match the filter
            files_to_process=$(find . -type f \( "${find_args[@]}" \))
        else
            # If no filter, process all files in all subdirectories (original behavior)
            files_to_process=$(find . -type f -path './*/*')
        fi

        log_with_timestamp "   - Running AWK script to generate CSVs..."
        if [ -n "$files_to_process" ]; then
            # Use xargs to safely pass the file list to awk
            echo "$files_to_process" | xargs awk -f "../$AWK_SCRIPT_NAME"
            log_with_timestamp "   - AWK script completed."
        else
            log_with_timestamp "   - ❌ WARNING: No log files found matching the filter criteria in '$folder_name'."
        fi
        ...
        cd ..
        log_with_timestamp "✅ Finished analysis for $folder_name"
        log_with_timestamp "--------------------------------"
    done
fi
