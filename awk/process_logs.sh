...
#   - gawk (GNU awk)
#   - python3, pip
#   - Python libraries: pandas, openpyxl (`pip install pandas openpyxl`)
#
# ===================================================================================
...
# Loop through all files ending with .log.txz in the current directory
for file in *.log.txz; do
    # Check if the file exists to avoid errors if no files are found
    [ -e "$file" ] || { echo "No .log.txz files found to process."; exit 1; }
    ...
    # 1. Create a directory for the extracted files
    echo "   - Creating directory: $folder_name"
    mkdir -p "$folder_name"

    # 2. Extract the archive into the new directory
    ...
    cd "$folder_name" || { echo "   - ❌ ERROR: Could not cd into $folder_name. Skipping."; cd ..; continue; }

    # 4. Run the AWK script to generate CSV files
    echo "   - Running AWK script to generate CSVs..."
    ...
    if awk -f "../$AWK_SCRIPT_NAME" */*; then
        echo "   - AWK script completed."
    else
        echo "   - ❌ WARNING: AWK script encountered an error."
    fi

    # 5. Run the Python script to combine CSVs into an Excel file
    echo "   - Running Python script to generate Excel file..."
    if python3 "../$PYTHON_SCRIPT_NAME"; then
        echo "   - Python script completed successfully."
    else
        echo "   - ❌ WARNING: Python script encountered an error."
    fi

    # 6. Return to the parent directory for the next loop iteration
    cd ..
...
echo "All log files have been processed."
